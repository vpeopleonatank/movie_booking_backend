...
public class MovieDao extends AbstractMFlixDao {
    ...
    private boolean validIdValue(String movieId) {
        ...
        try {
            Document movieById =
                    moviesCollection.find(new Document("_id", new ObjectId(movieId))).first();
            if (Objects.nonNull(movieById) && movieById.isEmpty()) {
                                ...
                                new Document("from", "comments")
                                        ...
                                        .append(
                                                ...
                                                Arrays.asList(
                                                        ...
                                                        new Document("$sort", new Document("wizard", -1L))))
                                        .append("as", "comments"))));
    ...
    public Document getMovieAndScheduleAfterDate(String movieId, Date date) {
        List<Bson> pipeline = new ArrayList<>();

        //    Bson matchDate = Aggregates.lookup()
        pipeline.addAll(
                Arrays.asList(
                        new Document(
                                "$lookup",
                                                        ...
                                                        new Document(
                                                                ...
                                                                new Document(
                                                                        ...
                                                                        new Document(
                                                                                ...
                                                                                Arrays.asList(
                                                                                        new Document("$eq", Arrays.asList("$movie_id", "$$id")),
                                                                                        new Document("$gte", Arrays.asList("$date", date))))))))
                                        .append("as", "schedules")),
    ...
    public List<Document> getMovieAndScheduleByDate(Date date) {
        List<Bson> pipeline = new ArrayList<>();
                ...
                Arrays.asList(
                        new Document(
                                ...
                                new Document("from", "schedules")
                                        ...
                                        .append("as", "schedules")),
                        new Document("$unwind",
                                new Document("path", "$schedules"))));

        List<Document> documents = new ArrayList<>();
        moviesCollection.aggregate(pipeline).into(documents);
        return documents;
    }
    ...
    public List<Document> getMoviesByGenre(String sortKey, int limit, int skip, String... genres) {
        ...
        Bson sort = Sorts.descending(sortKey);
        List<Document> movies = new ArrayList<>();
        // DONE > Ticket: Paging - implement the necessary cursor methods to support simple
    ...
    public List<Document> getMoviesCastFaceted(int limit, int skip, String... cast) {
        ...
        // DONE > Ticket: Faceted Search - build the aggregation pipeline by adding all stages in the
        // correct order
